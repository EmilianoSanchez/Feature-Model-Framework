...
public class HeuristicA implements HeuristicFunction<Problem<?, ?>> {
	...
	protected ObjectiveFunction buildHeuristicFunction(FeatureModel fmodel, ObjectiveFunction objectiveFunction,
		...
		if (objectiveFunction instanceof AggregateObjective) {
			heuristicFunction = new HeuristicA_AggregateObjective(fmodel, (AggregateObjective) objectiveFunction,
					optCriterion);
		} else {
			if (objectiveFunction instanceof MultiLinearPolynomialObjective)
				heuristicFunction = new HeuristicA_MultiLinearPolynomialObjective(fmodel,
